...
public class HistoricalDataNonAdminFragment extends BaseFragment {
    ...
    public static HistoricalDataNonAdminFragment newInstance() {
        Bundle args = new Bundle();
        HistoricalDataNonAdminFragment fragment = new HistoricalDataNonAdminFragment();
            ...
            if(time >= 0) {
                if (mLeScanner != null) {
                    if (i == Constants.Y_YEAR.length - 1) {
                        mLeScanner.addLastList(new ReadData(Constants.TYPE_HISTORICAL_CHART_DATA_NON_ADMIN_END,
                        ...
                        mLeScanner.addLastList(new ReadData(Constants.TYPE_HISTORICAL_CHART_DATA_NON_ADMIN,
                                ModbusData.buildReadRegsCmd(mLeScanner.getDeviceId(), ShourigfData.HistoricalChartDataInfo.REG_ADDR + time, ShourigfData.HistoricalChartDataInfo.READ_WORD)));
                    }
    ...
    private void receiveData(int type, byte[] data) {
        switch (type) {
            ...
            case Constants.TYPE_BATTERY:
                ...
                break;
            case Constants.TYPE_HISTORICAL_CHART_DATA_NON_ADMIN:
                ...
                break;
            case Constants.TYPE_HISTORICAL_CHART_DATA_NON_ADMIN_END:
