...
import com.shuorigf.bluetooth.streetlamp.adapter.HistoricalDataAdminAdapter;
...
public class HistoricalDataAdminDataFragment extends BaseFragment {
    @BindView(R.id.rv_historical_data_admin_data_content)
    RecyclerView mContentRv;
    @BindView(R.id.historical_data_admin_data_tablayout)
    TabLayout mTabLayout;
    @BindView(R.id.historical_data_admin_data_viewpager)
    ViewPager mViewPager;
    @BindView(R.id.tv_historical_data_non_admin_time_select)
    TextView mTimeSelectTv;
    @BindArray(R.array.historical_data_admin_chart_title)
    TypedArray mTabTitles;
    @BindArray(R.array.historical_data_admin_content_title)
    TypedArray mContentTitles;

    private BatteryTemperatureInfo mBatteryTemperatureInfo = new BatteryTemperatureInfo();
    private HistoricalDataAdminAdapter mHistoricalDataAdminAdapter;
    ...
    public static HistoricalDataAdminDataFragment newInstance() {
        Bundle args = new Bundle();
        HistoricalDataAdminDataFragment fragment = new HistoricalDataAdminDataFragment();
    ...
    public int getLayoutRes() {
        return R.layout.fragment_historical_data_admin_data;
    }
    ...
    private void initReadData() {
        if (mLeScanner != null) {
            ...
            mLeScanner.addFirstList(new ReadData(Constants.TYPE_BATTERY_TEMPERATURE,
                    ModbusData.buildReadRegsCmd(mLeScanner.getDeviceId(), ShourigfData.BatteryTemperatureInfo.REG_ADDR, ShourigfData.BatteryTemperatureInfo.READ_WORD)));
            ...
            if(time >= 0) {
                if (mLeScanner != null) {
                    if (i == Constants.Y_YEAR.length - 1) {
                        mLeScanner.addLastList(new ReadData(Constants.TYPE_HISTORICAL_CHART_DATA_ADMIN_MANAGER_END,
                        ...
                        mLeScanner.addLastList(new ReadData(Constants.TYPE_HISTORICAL_CHART_DATA_ADMIN_MANAGER,
                                ModbusData.buildReadRegsCmd(mLeScanner.getDeviceId(), ShourigfData.HistoricalChartDataInfo.REG_ADDR + time, ShourigfData.HistoricalChartDataInfo.READ_WORD)));
                    }
    ...
    private void readMonthChartData() {
       /* String curDate = mTimeSelectTv.getText().toString();
        ...
        for (int i = 0; i < Constants.Y_DAY.length; i++) {
         /*   String str = curDate + "-" + Constants.Y_DAY[i];
            Log.e("111", "readMonthChartData: " + str);
            int time = TimeUtils.getNowDaySpan(str,mYMDFormat);*/
            int time=7-i;
            if(time >= 0) {
                if (mLeScanner != null) {
    ...
    private void receiveData(int type, byte[] data) {
        switch (type) {
            ...
            case Constants.TYPE_BATTERY:
                ...
                mBatteryTemperatureInfo.mBatteryTemperature = batteryInfo.mBatteryTemperature;
                if (mHistoricalDataAdminAdapter != null) {
                    mHistoricalDataAdminAdapter.notifyDataSetChanged();
                }
                break;
            case Constants.TYPE_BATTERY_TEMPERATURE:
                ShourigfData.BatteryTemperatureInfo batteryTemperatureInfo = new ShourigfData.BatteryTemperatureInfo(data);
                mBatteryTemperatureInfo.mProtectivePlateTemperature = batteryTemperatureInfo.mProtectivePlateTemperature;
                mBatteryTemperatureInfo.mControlBoardTemperature = batteryTemperatureInfo.mControlBoardTemperature;
                if (mHistoricalDataAdminAdapter != null) {
                    mHistoricalDataAdminAdapter.notifyDataSetChanged();
                }
                break;
            case Constants.TYPE_HISTORICAL_CHART_DATA_ADMIN_MANAGER:
                ...
                break;
            case Constants.TYPE_HISTORICAL_CHART_DATA_ADMIN_MANAGER_END:
    ...
    private void initContent() {
        ...
        mHistoricalDataAdminAdapter = new HistoricalDataAdminAdapter(list, mBatteryTemperatureInfo);
        mContentRv.setItemViewCacheSize(list.size());
        mContentRv.setAdapter(mHistoricalDataAdminAdapter);
    }
