...
public class TimeUtils {
     ...
     * @param time   时间字符串
     * @param format 时间格式
     * @return 天数 比当前时间大则返回-1
     */
    public static int getNowDaySpan(final String time, final DateFormat format) {
        long now = string2Millis(getNowString(format), format);
        long millis = string2Millis(time, format);
        if (now < millis) {
            return -1;
        } else {
            return (int) ((now - millis) / TimeConstants.DAY);
        }
     ...
     * 获取向前7天加今天共八天的日期
     */
    public static String[] getOld7Day() {
        String[] old7Day=new String[8];
        for (int i = 0; i < 8; i++) {
           old7Day[i]=getOldDate(i-7).split("-")[2];
        }
        return old7Day;
    }
}